...
Когда Nginx передает запрос в Lapis, сначала определяется *путь*,
которому соответствует этот пакет. *Путь* - это паттерн, которому
под который подпадает URL. Пути сопоставляется *обработчик*, который
вызывается, когда приходит запрос, подходящий его пути.

Всем обработчикам в качестве аргумента передается
[*сам запрос*](#request-object).

Если обработчик возвращает строку, эта строка передается в браузер напрямую.
Если обработчик возвращает таблицу, то она используется в качестве
